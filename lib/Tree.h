...
namespace MicrosoftResearch { namespace Cambridge { namespace Sherwood
{
  ...
  public:
    ...
    void Apply(IDataPointCollection& data, std::vector<int>& leafNodeIndices)
    {
      CheckValid();
    ...
    static std::pair<DataPointIndex,DataPointIndex> PartitionNaN(std::vector<float>& keys, std::vector<unsigned int>& values, DataPointIndex i0, DataPointIndex i1, float threshold)
    {
        ...
        int i_left = (int)(i0);
        int j_right = (int)(i1 - 1),j_bad = (int)(i1 - 1);

        float key;
        unsigned int value;

        while(((i_left<j_right) | (i_left<j_bad)) & (j_right>=0) & (j_bad>=0))
        {
