...
namespace MicrosoftResearch { namespace Cambridge { namespace Sherwood
{
  ...
  struct Node // where F : IFeatureResponse where S: IStatisticsAggregator<S>
  {
    ...
    void InitializeLeaf(const S &trainingDataStatistics)
    {
      Feature = F();
      Threshold = 0.0f;
      bIsLeaf_ = true;
    ...
    void InitializeSplit(F feature, float threshold,const S &trainingDataStatistics)
    {
      bIsLeaf_ = false;
      bIsSplit_ = true;
      Feature = feature;
    ...
    void Serialize(std::ostream& o) const
    {
      ...
      TrainingDataStatistics.Serialize(o);
    }
