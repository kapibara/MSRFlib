...
namespace MicrosoftResearch { namespace Cambridge { namespace Sherwood
{
  ...
  public:
      ...
      progress_(progress)
    {
      ...
      validIndices_.resize(data.Count());
    ...
    void TrainNodesRecurse(std::vector<Node<F, S> >& nodes, NodeIndex nodeIndex, DataPointIndex i0, DataPointIndex i1, int recurseDepth)
    {
      ...
      for (int f = 0; f < parameters_.NumberOfCandidateFeatures; f++)
      {
        ...
        {
          int b = 0;
          while (b < nThresholds && responses_[validIndices_[i]] >= thresholds[b])
            b++;

          partitionStatistics_[b].Aggregate(data_, indices_[validIndices_[i]]);
        }
      ...
      std::vector<float>& thresholds)
    {
      ...
      if (i1 - i0 > parameters_.NumberOfCandidateThresholdsPerFeature)
      {
        ...
        for (int i = 0; i < nThresholds + 1; i++)
          quantiles[i] = responses[validIndices_[random.Next(0, lastValid_)]]; // sample randomly from all responses
